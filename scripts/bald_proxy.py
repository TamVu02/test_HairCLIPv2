...
class BaldProxy(torch.nn.Module):
    ...
    def forward(self, src_latent):
        ...
        with torch.no_grad():
            ...
            bald_target_img,_ = self.generator(latent_infer, input_code=True, return_latents=False) #self.generator([latent_infer], input_is_latent=True, return_latents=False)
            inv_source_img,_ = self.generator(src_latent, input_code=True, return_latents=False) #self.generator([src_latent], input_is_latent=True, return_latents=False)
