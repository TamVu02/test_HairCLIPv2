...
class BaldProxy(torch.nn.Module):
    ...
    def forward(self, src_latent):
        ...
        with torch.no_grad():
            ...
            bald_target_img, _ = self.generator.synthesis(latent_infer, noise_mode='const') #self.generator([latent_infer], input_is_latent=True, return_latents=False)
            inv_source_img, _ = self.generator.synthesis(src_latent, noise_mode='const') #self.generator([src_latent], input_is_latent=True, return_latents=False)
