...
def hairstyle_feature_blending(generator, seg, src_latent, src_feature, visual_mask, latent_bald, latent_global=None, latent_local=None, local_blending_mask=None):
    ...
    if latent_global is not None:
        bald_feature,_ = generator(latent_bald, input_code=True, return_latents=True)#generator([latent_bald], input_is_latent=True, return_latents=True, start_layer=0, end_layer=3)
        global_feature,_ = generator(latent_global, input_code=True, return_latents=True)#generator([latent_global], input_is_latent=True, return_latents=True, start_layer=0, end_layer=3)
        global_proxy,_ = generator(latent_global, input_code=True, return_latents=True)#generator([latent_global], input_is_latent=True, randomize_noise=False)
    ...
    if latent_local is not None:
        local_feature,_ = generator(latent_local, input_code=True, return_latents=True)#generator([latent_local], input_is_latent=True, return_latents=True, start_layer=0, end_layer=3)
    ...
    with torch.no_grad():
        img_gen_blend,_ = generator(src_latent, input_code=True, return_latents=True)#generator([src_latent], input_is_latent=True, randomize_noise=False, start_layer=4, end_layer=8, layer_in=src_feature)
...
def color_feature_blending(generator, seg, edited_hairstyle_img, src_latent, color_latent_in, latent_F):
    ...
    source_feature,_ = generator(src_latent, input_code=True, return_latents=True)#generator([src_latent], input_is_latent=True, randomize_noise=False, return_latents=True, start_layer=4, end_layer=6, layer_in=latent_F)
    color_feature,_ = generator(color_latent_in, input_code=True, return_latents=True)#generator([color_latent_in], input_is_latent=True, randomize_noise=False, return_latents=True, start_layer=4, end_layer=6, layer_in=latent_F)
