...
def hairstyle_feature_blending(generator, seg, src_latent, src_feature, visual_mask, latent_bald, latent_global=None, latent_local=None, local_blending_mask=None):
    ...
    if latent_global is not None:
        bald_feature = generator.decoder.synthesis(latent_bald, noise_mode='const')#generator.decoder.synthesis([latent_bald], input_is_latent=True, return_latents=True, start_layer=0, end_layer=3)
        global_feature = generator.decoder.synthesis(latent_global, noise_mode='const')#generator.decoder.synthesis([latent_global], input_is_latent=True, return_latents=True, start_layer=0, end_layer=3)
        global_proxy = generator.decoder.synthesis(latent_global, noise_mode='const')#generator.decoder.synthesis([latent_global], input_is_latent=True, randomize_noise=False)
    ...
    with torch.no_grad():
        img_gen_blend = generator.decoder.synthesis(src_latent, noise_mode='const')#generator.decoder.synthesis([src_latent], input_is_latent=True, randomize_noise=False, start_layer=4, end_layer=8, layer_in=src_feature)
